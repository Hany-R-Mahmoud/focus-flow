...
const steps = [
  {
    ...
    description: "Start with a clear intention so your attention has somewhere to land.",
    tone: "bg-amber-50 text-amber-700",
  },
  {
    icon: Clock3,
    title: "Stay with the work",
    description: "Run a focused timer and capture distractions without leaving your session.",
    tone: "bg-teal-50 text-teal-700",
  },
  {
    icon: Check,
    title: "See what happened",
    description: "Review your time, interruptions, and patterns while they are still useful.",
      ...
      <main>
          ...
          <div className="container grid gap-14 py-16 md:grid-cols-[minmax(0,0.9fr)_minmax(420px,1.1fr)] md:items-center md:py-24 lg:gap-20 lg:py-28">
            <div className="relative max-w-xl">
              <p className="mb-6 flex items-center gap-2 text-sm font-medium text-[var(--color-teal-dark)]">
                <span className="size-2 rounded-full bg-[var(--color-teal)]" aria-hidden="true" />
            ...
            <div className="relative mx-auto w-full max-w-2xl">
              <div className="absolute -inset-4 rounded-[2rem] bg-teal-50/60" aria-hidden="true" />
              <div className="relative overflow-hidden rounded-2xl border border-border bg-card shadow-[0_18px_50px_-28px_rgba(15,118,110,0.5)]">
                <div className="flex items-center justify-between border-b border-border px-5 py-4 sm:px-7">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">Tuesday, 10:30 AM</p>
                  </div>
                  <span className="rounded-full bg-teal-50 px-3 py-1 text-xs font-medium text-teal-700">Ready</span>
                </div>
                <div className="grid gap-6 p-5 sm:grid-cols-[1fr_0.9fr] sm:p-7">
                    ...
                    <div className="mt-10 flex items-center justify-center">
                      <div className="flex size-40 items-center justify-center rounded-full border-[11px] border-teal-100 border-t-[var(--color-teal)] sm:size-48">
                        <div className="text-center">
                          <p className="font-mono text-4xl font-semibold tracking-tight text-foreground">25:00</p>
                          <p className="mt-1 text-xs text-muted-foreground">until a clear break</p>
                        </div>
                      </div>
                    </div>
                    <button
                  ...
                  <div className="flex flex-col justify-between gap-6">
                    <div>
                      <p className="text-sm font-semibold">A little structure</p>
                      <p className="mt-2 text-sm leading-6 text-muted-foreground">Keep the plan visible, let the rest wait.</p>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 rounded-lg border border-border bg-background p-3">
                        <span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-md bg-amber-50 text-amber-700"><Sparkles size={15} aria-hidden="true" /></span>
                        <div><p className="text-xs font-medium">Intention</p><p className="mt-1 text-sm text-muted-foreground">Draft the project brief</p></div>
                      </div>
                      <div className="flex items-start gap-3 rounded-lg border border-border bg-background p-3">
                        <span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-md bg-sky-50 text-sky-700"><Link2 size={15} aria-hidden="true" /></span>
                        <div><p className="text-xs font-medium">Distractions</p><p className="mt-1 text-sm text-muted-foreground">3 notes captured privately</p></div>
                      </div>
                    </div>
                    <p className="text-xs leading-5 text-muted-foreground">A representative view of the focus workspace.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="border-y border-border/70 bg-slate-50/60">
          <div className="container py-20 sm:py-24">
            <div className="max-w-2xl">
              <h2 className="text-balance text-3xl font-semibold tracking-[-0.025em] sm:text-4xl">Your focus, in a simple loop.</h2>
              <p className="mt-4 text-lg leading-8 text-muted-foreground">Enough structure to begin and enough room to do the work your way.</p>
            </div>
            <div className="mt-12 divide-y divide-border border-y border-border">
              {steps.map((step) => {
                const Icon = step.icon;
                return (
                  <div key={step.title} className="grid gap-5 py-7 sm:grid-cols-[auto_0.8fr_1.2fr] sm:items-center sm:gap-8">
                    <span className={`flex size-11 items-center justify-center rounded-xl ${step.tone}`}><Icon size={20} aria-hidden="true" /></span>
                    <h3 className="text-xl font-semibold tracking-tight">{step.title}</h3>
                    <p className="max-w-lg text-base leading-7 text-muted-foreground">{step.description}</p>
                  </div>
                );
              })}
            </div>
        ...
        <section className="container py-20 sm:py-24">
          <div className="grid gap-10 rounded-2xl bg-[var(--color-teal-dark)] p-7 text-white sm:p-10 lg:grid-cols-[1fr_auto] lg:items-center lg:p-14">
            <div className="max-w-2xl">
              <div className="mb-5 flex size-11 items-center justify-center rounded-xl bg-white/10"><Users size={22} aria-hidden="true" /></div>
              <h2 className="text-balance text-3xl font-semibold tracking-[-0.025em] sm:text-4xl">Focus together, keep your own notes.</h2>
